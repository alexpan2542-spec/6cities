...
\Title{Cross-Product Agreement Does Not Arbitrate Reference Labels at the
Urban Built-Up/Vegetation Boundary: A Six-City Diagnostic}
...
so one product can arbitrate another's disputed pixels. We test this at the
urban built-up/vegetation boundary with 900 expert-labelled points---150 in
...
($Q \le 0.14$). Product consensus predicts the expert label no better than a
single product (0.44 against a 0.45 base rate), and all four sources agree on
only 18\% of points. The WorldCover error is directional---a ``built-up'' call
is wrong toward non-built about seven times in ten---so an agreement-filtered
label set systematically under-represents urban vegetation. Within this
existence characterisation---six cities, one
sensor---cross-product agreement should not serve on its own as a
pseudo-reference or high-confidence label at the urban boundary.}

\keyword{global land-cover products; ESA WorldCover; Dynamic World; Esri Land
Cover; map agreement; correlated error; urban vegetation boundary; accuracy
...
\item At the urban built-up/vegetation boundary in six middle- and lower-Yangtze cities,
none of ESA WorldCover, Dynamic World or Esri matches expert labels on even half
the boundary points (0.47, 0.45, 0.42), and product consensus does no better
(0.44 vs.\ a 0.45 base rate).
...
errors move almost in lockstep (Yule's $Q \approx 0.97$, against $\le 0.14$ for
either WorldCover pair).
\end{itemize}\vspace{10pt}
\noindent\textbf{What is the implication of the main finding?}
\begin{itemize}[labelsep=2.5mm,topsep=2pt]
\item At the urban boundary, cross-product agreement should not be used on its
own as a pseudo-reference, a high-confidence training label, or a quality
filter; the WorldCover error is directional---``built-up'' is wrong toward
non-built about seven times in ten---so agreement-filtered labels
under-represent urban vegetation.
...
%%  (C1-C5) or cross-reference changed; every edit information-equivalent.
...
is assumed to arbitrate a disputed pixel. Whether that assumption holds at the
urban built-up/vegetation boundary, the very place the products are weakest,
...
This paper makes five contributions. \textbf{C1}, the central one, is a direct
...
third product can arbitrate a disputed pixel. At the urban built-up/vegetation
boundary, in these six cities, it can fail: product consensus predicts the
expert label no better than a single product, and the two products that agree
...
independent corroboration. \textbf{C2}: the WorldCover boundary error is directional, not
symmetric noise---once WorldCover calls ``built-up'', the expert reads
non-built about seven times in ten---so an agreement-filtered label set
systematically under-represents urban vegetation. \textbf{C3}: the
...
active-learning method. \textbf{C4}, as supporting evidence, a
leave-one-city-out correction operator showing the disagreement carries a
cross-city signal with a bounded downstream footprint---not a correction
framework. \textbf{C5}: the resulting methodological caution---%
...
World folds urban vegetation into ``Built Area'', whereas WorldCover's
``Built-up'' nominally excludes it. \citet{xu2024} run a comparative
...
manual budget precisely on built-up/vegetation transitions, and (iii)~an
explicit test of the arbitration assumption, formalised as a testable violation
of conditional independence and supported by the joint error structure of the
three products, with class ontology, shared imagery and acquisition date ruled
out as cheap explanations. \emph{Why} the two products are coupled is left as a
...
\texttt{COPERNICUS/S2\_SR\_HARMONIZED} over the full calendar year (scenes
...
\item \textbf{Dynamic World} \citep{brown2022}---a near-real-time deep
  ...
  the class band (\texttt{GOOGLE/DYNAMICWORLD/V1}).
\item \textbf{Esri\,/\,Impact Observatory Land Cover} \citep{karra2021}---an
  ...
  of the time-series product (\texttt{ESRI\_Global-LULC\_10m\_TS}).
\end{itemize}
...
Dynamic World folds urban trees and grass into its ``Built Area'' class,
whereas WorldCover's ``Built-up'' nominally excludes vegetation
\citep{venter2022}. This split is taken up again in
Section~\ref{sec:disc-edge}. WorldCover enters every analysis; Dynamic World
and Esri enter only the four-way agreement, the independence diagnostic and the
correction-operator referee (Section~\ref{sec:diagnostic}).
...
%%  Labels defined: sec:methods sec:sampling sec:diagnostic sec:loco sec:eval
...
that contributes no labels of its own. Section~\ref{sec:eval} states the
metrics and the leakage controls that apply throughout.
...
(S2a)--(S2b) below, does not require the expert label to be ground truth; (S2c)
is the exception, since it asks how well the expert label is predicted, and is
accordingly capped by the expert's own reliability
(Section~\ref{sec:disc-scope}).
...
given the true class $Y$,
%
\begin{equation}\label{eq:condindep}
P_{\text{indep}}(A = B) = \sum_{j} P(Y = j) \sum_{c} P(A = c \mid Y = j)\,P(B = c \mid Y = j).
\end{equation}
%
The conditional class distributions are estimated in-sample, so they partly
absorb the true dependence they are meant to null out; this biases
$P_{\text{indep}}$ \emph{toward} the observed agreement, so the reported excess
is expected to understate the true value. Uncertainty is a 4000-sample point bootstrap with a
...
indicators; this describes the joint error structure of the three products
alone. \emph{(S2c) Arbitration value:} we evaluate $P(\text{expert} = \ell \mid
...
collapsed to two classes (built vs.\ rest, and built vs.\ vegetation with water
...
labelled points to their spectral neighbours \emph{within the same city}---the
prototype\,/\,pseudo-label rectification-and-expansion family, of which PRE
\citep{tong2024pre} is a representative instance. We adapted the same idea to
a within-city setting (our variant, \emph{Prototype Expansion}) and found it
leaks under spatial autocorrelation: once the evaluation points were properly
excluded from the neighbour search, the procedure produced no valid
expansions, and the apparent gain it showed without that control traced to a
classifier change, not to propagation (full leakage diagnostic in
Supplementary S5). We therefore define a correction operator trained on the
labelled points of $N - 1$ cities and transferred to a held-out city that
contributes no labels of its own---leave-one-city-out (LOCO)---which removes
this dependence by construction, and report it as a supporting result, not
...
training points. Five labelling schemes are compared: (B0)~raw WC labels;
(B2)~WC labels corrected by the transferred LOCO operator on boundary-gated,
confidence-gated points---\emph{the proposed method}; (B1)~a \emph{within-city
oracle} operator that does see city $c$'s expert labels (optimistic ceiling);
(B3)~the transferred operator without the boundary gate (ablation); and
(B4)~the same number of labels flipped to a random class (control). Each
classifier is scored for overall accuracy and Boundary Error on the held-out
spatial-block test set (against WC) and on the 150 expert points (against the
expert). A $\tau \times$ margin-quantile sensitivity sweep is in Supplementary
Table~S1.

\subsubsection*{Phase 3---multi-reference referee}

The corrected label map is evaluated directly, with no downstream classifier,
on the 150 held-out points of each city (all six now carry DW and Esri),
...
architecture can be mistaken for a method effect.
...
the downstream classifier never sees the held-out city's expert labels---its
150 boundary points are evaluation-only.

\subsubsection*{Controls and uncertainty}

A random-flip variant that changes the same number of labels to a random wrong
class accompanies every correction result. Binomial proportions use Wilson
95\% intervals. The independence statistics (Section~\ref{sec:diagnostic}(e))
use a 4000-sample point bootstrap and leave-one-city-out ranges.

\subsubsection*{Metrics}

Overall accuracy (OA), Cohen's $\kappa$, and Boundary Error (BE), defined as
the number of built $\leftrightarrow$ non-built confusions,
%
...
%%                   sec:sampling sec:diagnostic sec:loco sec:eval
...
and the largest internally consistent subset---expert, DW and Esri all
...
visually unambiguous, so the high-only rate is better read as an upper bound
than an unbiased estimate. Even so, two observations do not follow from that
selection effect alone. First, the result still refutes the ``noise on
ambiguous pixels'' reading---noise would not concentrate on the confident
calls. Second, the magnitude in Nanchang (42/42) and Wuhan (0.88) is larger
than a labelling-confidence artefact would plausibly produce. The
...
high-confidence expert $\neq$ WorldCover subset (281 points; the count before
the Hangzhou re-labelling was 293), we drew 30---five per city, in decreasing
...
matrix---disagrees as often as the built-up edge itself. That is consistent with
the documented
...
All statistics are on the $n = 900$ subset; the central contrast---excess
agreement and error dependence, below---does not require the expert label to
be ground truth. The arbitration-value check is the exception
...
Because the conditional class distributions are estimated in-sample, the
expected value is biased \emph{toward} independence, so $+$0.26 is expected to
understate the true excess (Section~\ref{sec:diagnostic}(e)).
...
against 0.78. The WC, DW interval still spans zero; WC, Esri is marginally
positive but nowhere near the DW/Esri coupling. WorldCover's errors are at most
weakly coupled to the two deep-segmentation products; those two err together.
This is a differential between the product pairs: a noisy or biased expert
reference would inflate the apparent error correlation of all three pairs alike,
not just one, so the contrast is robust to an imperfect expert rather than
merely independent of it. (The ``same wrong class'' rate is $\approx$ 0.87 for
...
against the marginal-independent value, Yule's $Q$ and Cohen's $\kappa$. Excess
...
agrees with that shared label 0.44 of the time [0.40, 0.47]---indistinguishable
from the 0.45 base rate of simply following DW. Requiring WorldCover to join
the consensus does not raise it (0.43 on $n = 391$; WC $=$ DW gives 0.45 on
$n = 436$). Product consensus carries essentially no additional information
about the expert label; the nominal ``$+$0.18 lift'' over the class prior is
just the 0.25 prior probability of the agreed-on class.

\paragraph{Confound checks.}
Three cheap explanations are ruled out. Collapsing the three classes to
built-vs-rest, or to built-vs-vegetation with water points dropped, leaves DW
$=$ Esri at 0.90--0.92 and the (DW $=$ Esri)\,$-$\,(expert $=$ DW) gap at
0.34--0.48, so the coupling is not an artefact of the three-class remap
...
against their deep-segmentation models \citep{zanaga2022,brown2022,karra2021}.
Its excess agreement is about half that of DW $=$ Esri and
its error $Q$ stays an order of magnitude smaller ($\le 0.14$ vs 0.97), so
shared imagery alone does not generate the coupling.
And the DW $=$ Esri rate is high in every scene type (0.83--0.98) and
\emph{highest} in the hardest transition zones---urban green space $\approx$
0.98, mixed\,/\,complex $\approx$ 0.96---the reverse of the pattern a shared
single-scene acquisition would produce (Supplementary Table~S4).
...
to predict the expert label. \emph{Why} the two are coupled---a shared
deep-segmentation
model family is the least speculative reading, with a shared training-label
lineage a further, less testable possibility---is taken up in
...
the margin over the majority-class baseline narrows to 0.05 in Hefei
(Supplementary Table~S5). Confident firings ($p_{\max} \ge
...
(0.07). A gate-sensitivity sweep (Supplementary Table~S1) shows why: a
materially larger downstream gain is available only by loosening the
confidence threshold and rewriting far more labels, at a real cost to WC
accuracy, while at the safe threshold used here the effect stays within noise.
A few dozen boundary-label corrections do not move a random forest trained on
$\approx$ 10,000 points, which is why Phase~3 evaluates the label map
directly.
...
At the urban built-up/vegetation boundary, in these six cities, agreement among
...
$Q = 0.97$, against $\le 0.14$ for either WorldCover pair), and their consensus
predicts the expert label no better than a single product (0.44 against a 0.45
...
boundary as a high-confidence signal---the C5 practice this paper set out to
...
where built-up meets vegetation---intra-urban green space (67.3\%) and the
built-up edge (66.1\%) are the two highest groups. This echoes the documented
...
schema artefact: Dynamic World folds urban vegetation into ``Built Area'' and
WorldCover's ``Built-up'' nominally excludes it \citep{venter2022}, yet
WorldCover still misses it at this rate. An agreement-filtered label set at the
urban fringe therefore under-represents vegetated cover, in the same direction
in all six cities.

The disagreement is nonetheless structured enough to carry a transferable
signal\label{sec:disc-loco}. A leave-one-city-out operator trained on five
...
(narrowest margin 0.05, in Hefei). As a direct label-map referee, it moves 83\% of the
points it changes toward the expert, against 33\% for random flips. But it
recovers only about one-fifth of a within-city oracle's gain, and at safe
thresholds its effect on a classifier trained on $\approx$ 10,000 points is
within between-seed noise. A few dozen boundary corrections are the right scale
for auditing a label map, not for retraining one.
...
needs product-internal information that is not published. The least speculative
reading, and the only one arguable from public documentation, is a shared model
family. Dynamic World and Esri are both deep semantic-segmentation models on
Sentinel-2 \citep{brown2022,karra2021}, whereas WorldCover---a
gradient-boosted-tree classifier with a rule-based post-process
\citep{zanaga2022}---is the architectural odd-one-out, whose errors are at most
weakly coupled to either ($Q \le 0.14$). A shared training-label lineage would produce the same
pattern but cannot be documented from the public record. Shared Sentinel-2
input is evidently not sufficient on its own, since WorldCover has it and does
not join the coupling. Which mechanism dominates is left to future work with
access to the training data and model internals; the statistical fact does not
depend on the answer---the two deep-segmentation products are not two
...
interpretable cases, not the shape spectral noise would take; and the sharpest
result---the $Q \approx 0.97$ for DW/Esri against $\le 0.14$ for the WorldCover
pairs---is a differential between product pairs that would survive an imperfect
expert, which would raise all pairs alike. An independent blind re-classification of 30 high-confidence
expert $\neq$ WorldCover points reproduced the expert label on 29 and the
WorldCover label on none (Appendix~\ref{app:recheck}).

\emph{Single-annotator reference.} Results that require the expert to be
correct---the confidence stratification, the error-direction analysis, the
arbitration PPV---are capped by its reliability. The independence contrast
(Section~\ref{sec:results-independence}) is the exception: it rests only on the
relationship among the three products' errors.

\emph{Confidence-stratum bias.} The rise in the expert $\neq$ WorldCover rate
on high-confidence labels (0.53 to 0.67) carries a known upward bias---an
annotator may reserve ``high'' when a product error is obvious---so it is an
upper bound, though Nanchang (42/42) and Wuhan (0.88) exceed what a labelling
...
\emph{Geographic and sensor scope.} Six middle- and lower-Yangtze cities, one climate, one
sensor: a bounded existence result, not a prevalence estimate, and in one of
...
six, so it is not a single-city artefact. Second, the Dynamic World/Esri
coupling follows from models trained once and applied globally---the strength
(0.87, $Q = 0.97$) will not carry over unchanged, but the mechanism plausibly
does. Replicating the boundary-aware sampling and the four-way diagnostic in
...
the reference label, at the urban built-up/vegetation boundary and on 900
...
to either ($Q \le 0.14$), and product consensus predicts the expert label no
better than a single product (0.44 against a 0.45 base rate). The WorldCover error is
directional---once it calls ``built-up'' the expert reads non-built about
seven times in ten---so an agreement-filtered label set at the urban fringe
under-represents vegetated cover, in the same direction in every city.

This is an existence characterisation for six cities and one sensor, not an
estimate of how often the behaviour occurs elsewhere. The specific coupling
strengths need not transfer, though the mechanism behind them---models trained
once and applied globally---plausibly does. The practical implication is
narrow: at this boundary, cross-product agreement should not be used on its own
...
(\texttt{ESA/WorldCover/v200}), Dynamic World
(\texttt{GOOGLE/DYNAMICWORLD/V1}), and Esri/Impact Observatory 10~m Land Cover
(\texttt{projects/sat-io/open-datasets/landcover/ESRI\_Global-LULC\_10m\_TS}).
The Sentinel-2 Level-2A surface-reflectance archive
(\texttt{COPERNICUS/S2\_SR\_HARMONIZED}) is likewise public. The 900
expert-interpreted boundary points, the boundary-aware sampling code, and the
diagnostic and leave-one-city-out analysis scripts are available from the
corresponding author on reasonable request and will be deposited at
\needsedit{\url{https://doi.org/XXXX}} upon publication.} %TODO repository DOI / access terms
...
only 0.389, inside the between-seed standard deviation (0.07). Table~\ref{tab:S1}
...
the $\approx$15,000 city labels rewritten.

\begin{table}[H]
\caption{Gate sensitivity, six-city mean. Source:
\texttt{docs/results\_snapshot/loco\_correction\_operator/phase2b\_sweep\_summary.csv}.\label{tab:S1}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
\begin{tabularx}{\fulllength}{CCCCC}
\toprule
\boldmath$\tau$ & \boldmath$q$ & \textbf{Points changed (of $\approx$15,000)}
...
15,000 labels, at a cost of $\approx$1.5 points of WC-OA (0.929 $\to$ 0.913).
At the safe operating thresholds used in the main text ($\tau \ge 0.85$) the
downstream effect is within between-seed noise. A few dozen boundary-label
corrections do not move a random forest trained on $\approx$10,000 points;
this is why the label-map referee (Section~\ref{sec:results-loco}, Phase~3)
evaluates the corrected map directly rather than through a retrained
classifier.
...
\texttt{docs/results\_snapshot/loco\_correction\_operator/phase1\_corrector\_transfer.csv}
...
that of DW $=$ Esri with error $Q \le 0.14$) uses quantities already
...
remap. Source:
\texttt{docs/results\_snapshot/independence\_diagnostic/S3a\_ontology\_confound.csv}.\label{tab:S3}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
...
classes. If the coupling came from the two products reading the same single
acquisition it should peak on the easy, stable classes and break down in the
transition zones; the observed pattern is the reverse. Source:
\texttt{docs/results\_snapshot/independence\_diagnostic/S3c\_coupling\_by\_scene.csv}.\label{tab:S4}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
\begin{tabularx}{\fulllength}{L{2}M{0.75}M{0.75}M{0.75}M{0.75}}
...
\texttt{data/analysis\_outputs/loco\_correction\_operator/phase1\_corrector\_transfer.csv}.\label{tab:S5}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
\begin{tabularx}{\fulllength}{lCCCCC}
...
WorldCover. Source: \texttt{docs/results\_snapshot/loco\_correction\_operator/}.\label{tab:S6}}
\centering
\small
...
Source: \texttt{docs/results\_snapshot/leak\_diagnostic/}
(\texttt{leak\_diagnostic.csv}, \texttt{spatial\_block\_results.csv},
\texttt{spatial\_block\_summary.csv}); generation script
\texttt{../gee-project2/scripts/spatial\_block\_validation.py}.

\subsection{Illustrative Example Chips: WorldCover Boundary Error (C1/C2)}\label{app:supp-s6}

Referenced from Section~\ref{sec:results-errordir}. Table~\ref{tab:phase3}
...
points across the six cities, chosen by a stated rule rather than
eyeballed---so a reader can see the error, not just read its rate. The
...
table (159 of 191 such points, 83\%). It illustrates C2 directly
...
looked at (\texttt{scripts/c2\_chip\_select.py}). A confidence filter is
...
count as errors---the counts behind Table~\ref{tab:phase3} and the 159/191
figure above are unaffected.

\paragraph{Imagery.} \texttt{COPERNICUS/S2\_SR\_HARMONIZED}, 2021,
\texttt{CLOUDY\_PIXEL\_PERCENTAGE < 20}, median---the same recipe as the
sampling pipeline that produced the 15,000-point candidate pool and the
WC/DW/Esri class calls being compared. Using any sharper or more recent
image source here would let a reader attribute the error to poor image
legibility rather than to the product classification itself, which is
exactly the confound the Section~\ref{sec:disc-scope} ``product error vs.\
S2 legibility'' discussion is written to rule out---so the chips deliberately
show the same imagery the products themselves were scored against, nothing
better.
...
only, the quantitative claim rests on Table~\ref{tab:phase3}
(Section~\ref{sec:results-errordir}).\label{fig:S5chips}}
\end{figure}

\needseditpar{Selection, fetch and assembly code for this figure will be
available at the repository DOI given in the Data Availability Statement upon
publication.} %TODO replace once repo DOI is set (see \dataavailability)
...
Evaluation of six global high-resolution land cover products over China.
...
Liao, S.; Wang, W.; Wang, C.; Ji, R.; Cui, A.; Chen, D.; Zhang, X.
...
Yangtze River Based on the PSR Model. \textit{Sustainability} \textbf{2023},
\textit{15}, 14098.
...
\url{https://data.apps.fao.org/map/catalog/} (accessed on 20 July 2026);
Google Earth Engine asset \texttt{FAO/GAUL/2015/level2}.
