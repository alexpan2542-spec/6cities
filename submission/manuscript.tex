...
(\path{projects/sat-io/open-datasets/landcover/ESRI_Global-LULC_10m_TS}).
...
code-plus-data repository: analysis code in \texttt{scripts/}; the six
cities' data (original 15{,}000-point pools through the final corrected
labels) in \texttt{data/cities/}; cross-city reference data (Dynamic
World/Esri second reference, scene grouping) in
\texttt{data/shared\_reference/}; script outputs in
\texttt{data/analysis\_outputs/}; and the inter-annotator blind re-check in
\texttt{data/interannotator\_recheck/}. This repository will be archived at
...
\path{loco_correction_operator/phase2b_sweep_summary.csv}.\label{tab:S1}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
...
remap. Source:
\path{independence_diagnostic/S3a_ontology_confound.csv}.\label{tab:S3}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
...
\path{independence_diagnostic/S3c_coupling_by_scene.csv}.\label{tab:S4}}
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\small
\begin{tabularx}{\fulllength}{L{2}M{0.75}M{0.75}M{0.75}M{0.75}}
...
\path{data/analysis_outputs/loco_correction_operator/phase1_corrector_transfer.csv}
...
WorldCover. Source: \path{loco_correction_operator/}.\label{tab:S6}}
\centering
\small
