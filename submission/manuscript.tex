...
systematically under-represents urban vegetation; the pattern is not a
single-city artifact, since a leave-one-city-out check reproduces the same
signal in a city holding out all of its own labels. \textbf{C3}: the
...
active-learning method. \textbf{C4}: the resulting methodological caution---%
...
$\approx$ 30\% of points are held out), so a training point and a test point
are never near-duplicates across the split. Five labelling schemes are
compared: (B0)~raw WC labels; (B2)~WC labels corrected by the transferred LOCO
...
expert labels (optimistic ceiling); (B3)~the transferred operator without the
boundary gate (ablation); and (B4)~the same number of labels flipped to a
random class (control). The correction operator never sees the held-out city,
...
\end{equation}
%
on the held-out spatial-block test set (against WC) and on the 150 expert
points (against the expert). A $\tau \times$ margin-quantile sensitivity sweep
...
\path{CLOUDY_PIXEL_PERCENTAGE < 20}, median---the same recipe as the
