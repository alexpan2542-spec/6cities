#!/usr/bin/env python3
"""
Hefei: human as WC assistant (no human↔WC adversarial overwrite)

Protocol on BAMS150 points
--------------------------
  human == WC  &  Conf=high  → keep WC label, weight=2.0, PE seeds
  human != WC  &  Conf=high  → discard (no special treatment; WC stays)
  Conf=med                   → use WC label, weight=0.5 (not PE seeds)
  Conf=low                   → discard

Comparators
-----------
  Baseline_RF / Baseline_MLP
  PE_high_adversarial   (old: overwrite WC with all high human labels)
  Assist_MLP            (weighted WC assist, no PE)
  Assist_PE             (weighted WC assist + PE from agree-high seeds)

Eval: pointwise vs original WorldCover Class.
Outputs -> data2/hefei_human_assist_wc/
"""
...
DATA2 = ROOT / "data2"
OUT_DIR = DATA2 / "hefei_human_assist_wc"
CITY = "Hefei"
...
def load_city() -> dict:
    ...
    y_all = feat3["Class"].astype(int).to_numpy() - 1  # 0-based
    ...
    lab_agree_high = wc[agree_high]  # == human
    pos_med = pos_all[med]
    lab_med = wc[med]

    # adversarial high pack (old protocol): all Conf=high human overwrite
    high_mask = np.asarray([c == "high" for c in conf], dtype=bool)
    pos_adv = pos_all[high_mask]
    lab_adv = human[high_mask]
    ...
    buckets = {
        ...
        "n_med": int(med.sum()),
        "n_low": int(low.sum()),
        "n_adv_high": int(high_mask.sum()),
        "conf_counts": {str(k): int(v) for k, v in Counter(conf).items()},
    }
    print(
        f"Hefei assist protocol: agree_high={buckets['n_agree_high']} (w={AGREE_HIGH_WEIGHT}), "
        f"disagree_high_drop={buckets['n_disagree_high']}, "
        f"med_WC_w={buckets['n_med']} (w={MED_WEIGHT}), low_drop={buckets['n_low']}; "
        f"adv_high={buckets['n_adv_high']}"
    )
    ...
    return {
        ...
        "idx_train": idx_train,
        "idx_test": idx_test,
        "pos_agree_high": pos_agree_high,
        "lab_agree_high": lab_agree_high,
        "pos_med": pos_med,
        "lab_med": lab_med,
        "pos_adv": pos_adv,
        "lab_adv": lab_adv,
...
def build_assist_xyw(X_all_s, y_all, idx_train, pos_agree_high, lab_agree_high, pos_med, lab_med):
    """
    Training set = WC train with per-point weights, plus any assist points
    not already in train (to avoid double-counting).
    """
...
def run_pe_adversarial(data, device, seed, epochs):
    X_all = data["X_all"]
    X_all_s = data["X_all_s"]
    y_all = data["y_all"]
    idx_train = data["idx_train"]
    idx_test = data["idx_test"]
    pos_seed = data["pos_adv"]
    lab_seed = data["lab_adv"]
    ...
    for k in K_LIST:
        ...
        print(
            f"  [seed={seed}] PE_high_adversarial K={k:<2d} exp={len(exp_idx):<3d} "
...
def run(device: str, seeds: list[int], epochs: int):
    ...
    for seed in seeds:
        ...
        # Adversarial PE_high (passport)
        best_adv = run_pe_adversarial(data, device, seed, epochs)
        rows.append(
            {
                "City": CITY,
                "Method": "PE_high_adversarial",
                "Seed": seed,
                **best_adv["m"],
                "BestK": best_adv["k"],
                "n_expanded": best_adv["n_exp"],
                "n_seed": int(len(data["pos_adv"])),
            }
        )
        print(
            f"  [seed={seed}] PE_high_adversarial   OA={best_adv['m']['OA']:.4f} "
            f"BE={best_adv['m']['Boundary Error']} K={best_adv['k']} exp={best_adv['n_exp']}"
        )
...
def summarize(df: pd.DataFrame, buckets: dict) -> tuple[str, pd.DataFrame]:
    ...
    lines = [
        "# Hefei: human as WC assistant\n",
        "\n## Protocol\n",
        f"- agree_high (human==WC & h): weight={AGREE_HIGH_WEIGHT}, PE seeds "
        f"(n={buckets['n_agree_high']})\n",
        f"- disagree_high (human!=WC & h): discard (n={buckets['n_disagree_high']})\n",
        f"- med: WC label, weight={MED_WEIGHT} (n={buckets['n_med']})\n",
        f"- low: discard (n={buckets['n_low']})\n",
        f"- comparator PE_high_adversarial uses all high human overwrite "
        f"(n={buckets['n_adv_high']})\n",
        "\n## Mean over seeds\n\n",
        overall.to_string(index=False),
        "\n\n## Reading\n",
        "- If Assist_PE ≫ PE_high_adversarial → assisting WC beats overwriting WC.\n",
        "- If Assist_PE ≈ Baseline_MLP → weighting alone does little under WC eval.\n",
        "- If Assist_PE < PE_high_adversarial → conflict high seeds were carrying the old gain.\n",
    ]
